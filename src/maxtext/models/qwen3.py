...
from maxtext.layers.normalizations import (
    RMSNorm,
    l2norm,
    Qwen3NextRMSNorm,
...
def naive_jax_chunk_gated_delta_rule(
    ...
    attn = (
        -jnp.matmul(k_beta_c, jnp.swapaxes(key_c, -1, -2), precision=prec) * decay_mask
    )
    ...
    k_cumdecay = jnp.matmul(
        attn, (k_beta_c * jnp.expand_dims(jnp.exp(g_cumsum), -1)), precision=prec
    )

    output_final_state = initial_state is not None
    if initial_state is None:
        last_recurrent_state = jnp.zeros(
            (batch_size, num_heads, k_head_dim, v_head_dim), dtype=value_intra.dtype
        )
    else:
    ...
    def naive_gdn_scan_body(prev_state, x):
        ...
        attn_i = (
            jnp.matmul(q_i, jnp.swapaxes(k_i, -1, -2), precision=prec) * decay_mask_i
        )
        ...
        attn_inter = jnp.matmul(
            q_i * jnp.expand_dims(g_i_exp, -1), last_recurrent_state, precision=prec
        )
        ...
        g_diff_exp = jnp.expand_dims(
            jnp.exp(jnp.expand_dims(g_i[..., -1], -1) - g_i), -1
        )
        k_i_g_diff = k_i * g_diff_exp

        update_term = jnp.matmul(
            jnp.swapaxes(k_i_g_diff, -1, -2), v_new, precision=prec
        )
    ...
    final_state, core_attn_out_stacked = naive_gdn_scan(
        naive_gdn_scan_body, last_recurrent_state, xs
    )
...
def jax_chunk_gated_delta_rule(
    ...
    u_chunks = jnp.matmul(
        A, v_beta.astype(jnp.float32), precision=jax.lax.Precision.HIGHEST
    )
...
def jax_ar_gated_delta_rule(
    ...
    scale = jax.lax.rsqrt(jnp.array(q.shape[-1], dtype=jnp.float32)).astype(
        compute_dtype
    )
    q = q * scale
    ...
    v_prime = jnp.matmul(
        k_cumdecay, state, precision=jax.lax.Precision.HIGHEST
    ).squeeze(-2)
    ...
    attn_intra = jnp.sum(
        q.astype(jnp.float32) * k.astype(jnp.float32), axis=-1, keepdims=True
    )
...
class Qwen3NextGatedDeltaNet(nnx.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self._gdn_replicate_expert = (
            os.environ.get("MAXTEXT_GDN_REPLICATE_EXPERT", "False").lower() == "true"
        )
        cfg = self.config
        ...
        if model_mode != MODEL_MODE_TRAIN and inputs_shape is not None:
            ...
            self.cache = (
                None  # No cache for train mode or when inputs_shape not provided
            )
        ...
        def a_log_init(key, shape, dtype=jnp.float32):
            ...
            a_vals = jax.random.uniform(
                key, shape=shape, dtype=dtype, minval=1e-9, maxval=16.0
            )
            return jnp.log(a_vals)

        self.A_log = nnx.Param(
            a_log_init(rngs.params(), (self.num_v_heads,), dtype=cfg.weight_dtype)
        )
        self.dt_bias = nnx.Param(
            nnx.initializers.ones(
                rngs.params(), (self.num_v_heads,), dtype=cfg.weight_dtype
            )
        )
    ...
    def _call_impl(
        ...
        **kwargs,
        ...
        if self.mesh is not None:
            ...
            qkvz_pspec = logical_to_mesh_axes(
                (KV_BATCH, None, KV_HEAD, None), mesh=self.mesh, rules=logical_rules
            )
        ...
        split_indices_qkvz = [
            ...
            2 * self.head_k_dim
            + (self.v_heads_per_k_head * self.head_v_dim),  # 2 * D_k + V_per_K * D_v
        ]
        # query: (B, S, H_k, D_k)
        # key: (B, S, H_k, D_k)
        ...
        if use_paged_state:
            ...
            try:
                ...
                from tpu_inference.utils import (
                    get_mesh_shape_product,
                )  # pylint: disable=import-outside-toplevel
                from jax.sharding import (
                    PartitionSpec as P_spec,
                )  # pylint: disable=import-outside-toplevel
            ...
            attn_head = (
                ShardingAxisName.MODEL
                if self._gdn_replicate_expert
                else ShardingAxisName.ATTN_HEAD
            )
            ...
            q_flat = query.reshape(
                num_tokens, self.key_dim
            )  # (T, key_dim) sharded on ATTN_HEAD
            k_flat = key.reshape(num_tokens, self.key_dim)
            v_flat = value_raw.reshape(
                num_tokens, self.value_dim
            )  # (T, value_dim) sharded on ATTN_HEAD
            ...
            if conv_state.shape[0] != batch:
                if conv_state.shape[0] == 1:
                    conv_state = jnp.broadcast_to(
                        conv_state, (batch,) + conv_state.shape[1:]
                    )
            ...
            if recurrent_state.shape[0] != batch:
                if recurrent_state.shape[0] == 1:
                    recurrent_state = jnp.broadcast_to(
                        recurrent_state, (batch,) + recurrent_state.shape[1:]
                    )
                elif recurrent_state.shape[0] < batch:
                    pad_amt = batch - recurrent_state.shape[0]
                    recurrent_state = jnp.pad(
                        recurrent_state, ((0, pad_amt), (0, 0), (0, 0), (0, 0))
                    )
                else:
        ...
        if seq_len == 1 and model_mode == MODEL_MODE_AUTOREGRESSIVE:
            ...
            g_beta_pspec = logical_to_mesh_axes(
                (KV_BATCH, None, KV_HEAD), mesh=self.mesh, rules=logical_rules
            )
        ...
        if model_mode != MODEL_MODE_TRAIN and active_cache is not None:
            ...
            if next_conv_state.shape[0] != orig_cache_batch:
                if next_conv_state.shape[0] == 1:
                    next_conv_state = jnp.broadcast_to(
                        next_conv_state, (orig_cache_batch,) + next_conv_state.shape[1:]
                    )
                    next_recurrent_state = jnp.broadcast_to(
                        next_recurrent_state,
                        (orig_cache_batch,) + next_recurrent_state.shape[1:],
                    )
                elif next_conv_state.shape[0] < orig_cache_batch:
                    pad_amt = orig_cache_batch - next_conv_state.shape[0]
                    next_conv_state = jnp.pad(
                        next_conv_state, ((0, pad_amt), (0, 0), (0, 0))
                    )
                    next_recurrent_state = jnp.pad(
                        next_recurrent_state, ((0, pad_amt), (0, 0), (0, 0), (0, 0))
                    )
                else:
...
class Qwen3NextSparseMoeBlock(nnx.Module):
    ...
    def __init__(
        self, config: Config, mesh: Mesh, quant: None | Quant = None, *, rngs: nnx.Rngs
    ):
        self.config = config
        self.mesh = mesh
        self.quant = quant
    ...
    def __init__(
        ...
        *,
        ...
        is_full_attention_layer = (
            self.layer_idx + 1
        ) % cfg.inhomogeneous_layer_cycle_interval == 0
...
class Qwen3OmniMoeVisionPatchMerger(nnx.Module):
    ...
    def __call__(self, hidden: Array) -> Array:
        ...
        hidden = hidden.reshape(
            batch_size, num_blocks, tokens_per_block * base_hidden_size
        )
        ...
        if self.use_postshuffle_norm:
            ...
            hidden = hidden_unmerged.reshape(
                batch_size, num_blocks, tokens_per_block * base_hidden_size
            )
...
class Qwen3OmniMoeVisionPatchEmbed(nnx.Module):
    ...
    def __call__(
        self, hidden_states: Array, video_mask: Array | None = None
    ) -> tuple[Array, Array | None]:
        """
        Args:
        ...
        seq_len = (
            hidden_states.shape[1] * hidden_states.shape[2] * hidden_states.shape[3]
        )
        ...
        if video_mask is not None:
            ...
            attention_mask = (
                video_mask.reshape(video_mask.shape[0], -1, mask_patch_elements)
                .max(axis=-1)
                .astype(jnp.int32)
            )
...
class Qwen3OmniMoeVisionBlock(nnx.Module):
    ...
    def __init__(self, config: Config, *, mesh=None, rngs: nnx.Rngs = None):
        ...
        self.ln1 = nnx.LayerNorm(
            num_features=hs, epsilon=config.normalization_layer_epsilon, rngs=rngs
        )
...
class Qwen3OmniMoeVisionEncoder(nnx.Module):
    ...
    def __call__(
        ...
        if video_mask is not None:
            ...
            attention_mask = (
                video_mask.reshape(batch_size, -1, mask_patch_elements)
                .max(axis=-1)
                .astype(jnp.int32)
            )
        ...
        if attention_mask is not None and batch_size != 1:
            raise ValueError(
                "Padded Qwen3-Omni vision encoding currently supports batch size one."
            )
        if video_grid_thw is not None:
            grid = (
                video_grid_thw[0]
                if getattr(video_grid_thw, "ndim", 1) == 2
                else video_grid_thw
            )
        ...
        if attention_mask is not None and valid_grid is not None:
            ...
            valid_indices = jnp.nonzero(attention_mask[0], size=math.prod(valid_grid))[
                0
            ]
...
class Qwen3OmniAudioEncoderLayer(nnx.Module):
    ...
    def __init__(self, config: Config, mesh: Mesh, *, rngs: nnx.Rngs = None):
        ...
        self.self_attention_audio = Attention(
            ...
            head_dim=self.config.d_model_for_audio
            // self.config.encoder_attention_heads_for_audio,
        ...
        self.AudioMLP = MlpBlock(
            ...
            kernel_init=max_initializers.nd_dense_init(
                self.config.dense_init_scale, "fan_in", "truncated_normal"
            ),
...
class Qwen3OmniAudioEncoder(nnx.Module):
    ...
    def __call__(
        ...
        audio_chunks = audio_features.reshape(
            batch_size, num_mel_bins, num_chunks, chunk_size
        )
        audio_chunks = audio_chunks.transpose(0, 2, 1, 3)
        audio_chunks = audio_chunks.reshape(
            batch_size * num_chunks, num_mel_bins, chunk_size
        )
        ...
        pos_emb = jnp.broadcast_to(
            pos_emb[None, :, :],
            (batch_size * num_chunks, seq_len_per_chunk, self.config.d_model_for_audio),
        )
