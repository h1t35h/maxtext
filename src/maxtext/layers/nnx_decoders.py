    ...
    def __init__(
        ...
        *,
        ...
        self.self_attention = Attention(
            ...
            prefill_cache_axis_order=tuple(
                map(int, cfg.prefill_cache_axis_order.split(","))
            ),
        ...
        self.dropout = linears.Dropout(
            rate=cfg.dropout_rate, rngs=rngs, broadcast_dims=(-2,)
        )
...
def deepstack_process(hidden_states, bidirectional_mask, visual_embeds):
    ...
    visual_token_idx = (
        jnp.cumsum(bidirectional_mask, axis=1) - 1
    )  # [batch, seq_len], 0-indexed
    ...
    visual_embeds_scattered = visual_embeds[
        batch_idx, visual_token_idx, :
    ...
    def __init__(
        ...
        *,
        ...
        for i in range(num_layers):
            layer = layer_cls(
                config=config, mesh=mesh, quant=quant, model_mode=model_mode, rngs=rngs
            )
    ...
    def __init__(
        ...
        *,
        ...
        def create_layer_fn(rng):
            return layer_cls(
                config=config, mesh=mesh, quant=quant, model_mode=model_mode, rngs=rng
            )
...
class NNXDecoder(nnx.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.dropout = linears.Dropout(
            rate=config.dropout_rate, rngs=rngs, broadcast_dims=(-2,)
        )
        self.positional_embedding = PositionalEmbedding(
            embedding_dims=config.base_emb_dim
        )
        ...
        self.is_gemma4_small = (
            self.config.decoder_block == DecoderBlockType.GEMMA4_SMALL
        )
    ...
    def _init_decoder_layers(self, decoder_block_classes, rngs, mesh):
        ...
        if self.is_gemma4_small:
            ...
            if config.using_pipeline_parallelism or config.scan_layers:
                raise ValueError(
                    "gemma4_small (Gemma4 E2B/E4B) does not support pipeline parallelism or scan_layers."
                )
    ...
    def _init_scanned_deepseek_engram(self, dense_cls, moe_cls, rngs):
        ...
        while current_idx < config.first_num_dense_layers:
            if current_idx in config.engram_layers:
                ...
                next_boundary = self._find_next_boundary(
                    current_idx, config.first_num_dense_layers, config.engram_layers
                )
        ...
        while current_idx < config.num_decoder_layers:
            if current_idx in config.engram_layers:
                ...
                next_boundary = self._find_next_boundary(
                    current_idx, config.num_decoder_layers, config.engram_layers
                )
    ...
    def _init_sequential_generic(self, decoder_block_classes, rngs):
        ...
        for lyr in range(config.num_decoder_layers):
            ...
            if config.decoder_block == DecoderBlockType.GEMMA3:
                ...
                layer_kwargs = {
                    "attention_type": gemma4.get_attention_type(layer_id=lyr)
                }
            elif config.decoder_block == DecoderBlockType.LLAMA4:
                layer_kwargs = {
                    "is_nope_layer": llama4.determine_is_nope_layer(
                        lyr, self.config.nope_layer_interval
                    ),
                    "is_moe_layer": llama4.determine_is_moe_layer(
                        lyr, self.config.interleave_moe_layer_step
                    ),
                }
                ...
                layer_kwargs = {
                    "attention_type": gpt_oss.get_attention_type(layer_id=lyr)
                }
            elif config.decoder_block == DecoderBlockType.OLMO3:
                layer_kwargs = {
                    "attention_type": olmo3.get_attention_type(layer_id=lyr)
                }

            self._create_and_register_layer(
                layer_cls, rngs, "layers", lyr, **layer_kwargs
            )
    ...
    def _init_gemma4_small_layers(self, rngs):
        ...
        if cfg.hidden_size_per_layer_input > 0 and cfg.vocab_size_per_layer_input > 0:
            self.per_layer_embedder = gemma4_small.Gemma4SmallPLE(
                config=cfg, mesh=self.mesh, rngs=rngs
            )

        layer_types = gemma4_small.build_layer_types(
            cfg.num_decoder_layers, cfg.model_name
        )
    ...
    def _apply_layer_with_remat(
        self, layer: nnx.Module, y: jax.Array, policy: Any, prevent_cse: bool, **kwargs
    ):
    ...
    def _apply_layers_sequentially(
        self, layers, x_in, *args, length: int, kv_caches_stacked=None, **kwargs
    ):
        ...
        def _extract_matching_state(template, full):
            if isinstance(template, nnx.State):
                return nnx.State(
                    {
                        k: _extract_matching_state(v, full[k])
                        for k, v in template.items()
                    }
                )
            elif isinstance(template, dict):
                return {
                    k: _extract_matching_state(v, full[k]) for k, v in template.items()
                }
            return full
        ...
        def nnx_scan_layer(carry, scanned_vars):
            ...
            if dynamic_graph_init:
                new_graphdef, updated_params, updated_state = nnx.split(
                    layer, nnx.Param, ...
                )
        ...
        layer_fn_wrapped = jax.checkpoint(
            nnx_scan_layer, policy=policy, prevent_cse=prevent_cse
        )
        ...
        if use_kv:
            ...
            final_carry, scanned_state = nnx_decoder_scan(
                layer_fn_wrapped, x_in, params, state
            )
            returned_kv_stacked = None

            # Ensure metadata rank matches the stacked values
            scanned_state = maxtext_utils_nnx.nnx_add_scan_axis(
                scanned_state, "layers", 0
            )

            if scan_axis != 0:
                new_params, new_rest = scanned_state.split(nnx.Param, ...)
                new_params = maxtext_utils_nnx.nnx_sync_moveaxis(
                    new_params, 0, scan_axis
                )
    ...
    def get_norm_layer(self, num_features: int, rngs: nnx.Rngs):
        ...
        if self.config.decoder_block in {
            ...
            raise ValueError(
                f"Incorrect decoder_block name {self.config.decoder_block.value=}"
            )
    ...
    def _apply_embedding(
        ...
        y = shared_embedding(
            decoder_input_tokens.astype("int32"), model_mode=model_mode
        )
        ...
        if multimodal_input is not None:
            ...
            bidirectional_mask_video = getattr(
                multimodal_input, "bidirectional_mask_video", None
            )
            ...
            if image_embeddings is not None and cfg.use_multimodal:
                if cfg.model_name in {
                    ...
                    raise ValueError(
                        f"Unsupported model_name for multimodal: {cfg.model_name}"
                    )
            ...
            if video_embeddings is not None and cfg.use_multimodal:
                if cfg.model_name in {
                    ...
                    raise ValueError(
                        f"Unsupported model_name for video: {cfg.model_name}"
                    )
            ...
            if audio_embeddings is not None and cfg.use_audio:
                if cfg.model_name in {"qwen3-omni-30b-a3b"}:
                    ...
                    raise ValueError(
                        f"Unsupported model_name for audio: {cfg.model_name}"
                    )
    ...
    def apply_output_head(self, shared_embedding, y, deterministic, model_mode):
        ...
        if cfg.logits_via_embedding:
            ...
            logits = attend_on_embedding(
                y, embedding_table, attend_dtype, self.config, out_sharding
            )
    ...
    def _build_linen_params(self, moe_stack: nnx.Module) -> dict:
        ...
        return {
            "pre_self_attention_layer_norm": state_dict[
                "pre_self_attention_layer_norm"
            ],
            "post_self_attention_layer_norm": state_dict[
                "post_self_attention_layer_norm"
            ],
            "self_attention": state_dict["self_attention"],
            "DeepSeekMoeBlock_0": state_dict.get(
                "moe_block", state_dict.get("DeepSeekMoeBlock_0")
            ),
        }
    ...
    def _apply_scanned_chunk(
        self, y, current_idx, next_boundary, layer_stack, *args, **kwargs
    ):
        ...
        if scan_length > 0:
            ...
            chunk_params = jax.tree.map(
                lambda x: jax.lax.dynamic_slice_in_dim(
                    x, current_idx, scan_length, axis=scan_axis
                ),
                params,
            )
            chunk_rest = jax.tree.map(
                lambda x: jax.lax.dynamic_slice_in_dim(
                    x, current_idx, scan_length, axis=0
                ),
                rest,
            )
            ...
            updated_params = jax.tree.map(
                lambda s, new_s: jax.lax.dynamic_update_slice_in_dim(
                    s, new_s, current_idx, axis=scan_axis
                ),
                params,
                new_params,
            )
    ...
    def _apply_interleaved_scanned_layers(
        self, y, layer_prefix, start_idx, end_idx, engram_indices, *args, **kwargs
    ):
        ...
        while current_idx < end_idx:
            if current_idx in engram_indices:
                ...
                next_boundary = self._find_next_boundary(
                    current_idx, end_idx, engram_indices
                )
    ...
    def __call__(
        ...
        if cfg.using_pipeline_parallelism:
            ...
            if self.is_deepseek:
                ...
                with self.mesh, nn.partitioning.axis_rules(logical_axis_rules_pp_as_dp):
                    if cfg.scan_layers:
                        if (
                            getattr(self, "dense_layers", None) is not None
                            and cfg.first_num_dense_layers > 0
                        ):
                        ...
                        if (
                            hasattr(self, "moe_layers_outside_pipeline")
                            and self.moe_layers_outside_pipeline is not None
                        ):
                            num_moe_outside = (
                                cfg.num_decoder_layers - cfg.first_num_dense_layers
                            ) - cfg.pipeline_parallel_layers
                ...
                if hasattr(self, "layers_outside_pipeline") or hasattr(
                    self, "num_layers_outside_pipeline"
                ):
                    logical_axis_rules_pp_as_dp = (
                        sharding.logical_axis_rules_pp_act_as_dp(cfg.logical_axis_rules)
                    )
                    with (
                        self.mesh,
                        ...
                        if cfg.scan_layers and hasattr(self, "layers_outside_pipeline"):
                            remaining = (
                                cfg.num_decoder_layers - cfg.pipeline_parallel_layers
                            )
                            y, self.layers_outside_pipeline, _ = (
                                self._apply_layers_sequentially(
                        ...
                        elif (not cfg.scan_layers) and hasattr(
                            self, "num_layers_outside_pipeline"
                        ):
                            for i in range(self.num_layers_outside_pipeline):
            ...
            if self.is_gemma4_small:
                ...
                if self.is_deepseek:
                    ...
                    scan_length = int(
                        cfg.num_decoder_layers / cfg.inhomogeneous_layer_cycle_interval
                    )
                ...
                def pure_layer_fn(graphdef, state_in, y_in, kv_in):
                    ...
                    out_y, out_kv = merged_layer(
                        y_in, *layer_args, kv_cache=kv_in, **layer_kwargs
                    )
                ...
                for lyr, layer in enumerate(self.layers):
                    ...
                    y, kv_cache, new_state = checkpointed_fn(
                        graphdef, state, y, kv_cache
                    )
                    nnx.update(layer, new_state)
                    ...
                    if deepstack_visual_embeds is not None and lyr < len(
        ...
        # Instead, we keep track on the hidden states, which has smaller size compared to full logits
            ...
            logits = self.apply_output_head(
                shared_embedding, hidden_state, deterministic, model_mode
            )
    ...
    def _apply_gemma3_scanned_blocks(
        ...
        if num_remaining_layers > 0:
            ...
            if kv_caches is not None:
                ...
                remainder_kv = tuple(
                    kv_caches[start_idx : start_idx + num_remaining_layers]
                )
            ...
            checkpointed_gemma_fn = jax.checkpoint(
                pure_gemma_fn, policy=policy, prevent_cse=prevent_cse
            )

            graphdef, state = nnx.split(self.layers_remainder)
            y, updated_remainder_kv, new_state = checkpointed_gemma_fn(
    ...
    def _apply_gemma4_small_layers(
        ...
        for lyr in range(cfg.num_decoder_layers):
            ...
            ple_slice = (
                per_layer_inputs[..., lyr, :] if per_layer_inputs is not None else None
            )
